...
export function initializeSearch(searchInput, searchResults, searchIndex, options = {}) {
    ...
    function displaySearchResults(results, query) {
        ...
        // Add results count with proper ARIA
        const countDiv = document.createElement("div")
        countDiv.className = "search-results-count"
        countDiv.setAttribute("role", "status")
        ...
        Object.entries(groupedResults).forEach(([type, typeResults]) => {
            ...
            // Add type header with proper semantics
            const typeHeader = document.createElement("div")
            typeHeader.className = "search-results-type-header"
            typeHeader.setAttribute("role", "heading")
            typeHeader.setAttribute("aria-level", "2")
        ...
        // Update the search results container with proper ARIA
        const searchResultsContainer = searchResults.querySelector(".search-results")
        if (searchResultsContainer) {
            searchResultsContainer.setAttribute("aria-label", `${results.length} search results for ${query}`)
        }
    }
    ...
    function createResultItem(result, query) {
        ...
        // FIXED: Make focusable for proper tab navigation within modal
        resultItem.setAttribute("role", "option")
        resultItem.setAttribute("tabindex", "0") // Changed from -1 to 0 for tab navigation
        resultItem.setAttribute("aria-describedby", `result-${result.id}-desc`)
        ...
        <div class="search-result-content">
            <div class="search-result-title" role="heading" aria-level="3">${highlightedTitle}</div>
            <div class="search-result-path">${escapeHtml(result.path)}</div>
            <div class="search-result-preview" id="result-${result.id}-desc">${highlightedContent}</div>
            ${additionalInfo.length > 0 ? `<div class="search-result-meta">${additionalInfo.join(" • ")}</div>` : ""}
        </div>
        <div class="search-result-type-badge" aria-hidden="true">${result.type || "content"}</div>
    `
        ...
        // FIXED: Add proper keyboard navigation
...
export async function loadSearchIndex() {
    const searchIndexUrl = "/content/themes/editorial/assets/json/search-index.json"
    ...
    try {
        ...
        throw error // Re-throw so SearchModal can handle it
    }
}
